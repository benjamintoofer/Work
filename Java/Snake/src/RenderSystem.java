...
public class RenderSystem extends Canvas {
    ...
    //Leap motion conteoler and listener
    private  SampleListener listener;
    private  Controller control;
    private Vector normHand;
    ...
    public void tick()
    {
        //System.out.println(control.frame().gestures().);
        //directionGesture();
        switch (currentState)
        {
            case MENU_STATE:
    ...
    private void renderHandCursor(GraphicsContext gc)
    {
        ...
        for(Hand h: frame.hands())
        {
            ...
            normHand = box.normalizePoint(h.palmPosition());
            gc.setFill(Color.YELLOW);
            gc.fillOval(width * normHand.getX(),height - (height * normHand.getY()), 20,20);
            //System.out.println((width * vec.getX())+" - "+(height * vec.getY()));
        }
    }
        ...
        public void onFrame(Controller controller) {
            ...
            if(currentState == GAME_STATE)
            {
                for(Finger f: frame.fingers())
                {
                    ...
                    if(f.tipVelocity().getY() > max)
                    {
                        max = f.tipVelocity().getY();
                    }
                    if(f.tipVelocity().getY() < min)
                    {
                        min = f.tipVelocity().getY();
                    }
                    //System.out.println(max+" "+min);
                    if(Math.abs(f.tipVelocity().getX()) > 1000)
                    {
                        if(f.tipVelocity().getX() > 0)
                        {
                    ...
                    if(Math.abs(f.tipVelocity().getY()) > 1000)
                    {
                        if(f.tipVelocity().getY() > 0)
                        {
